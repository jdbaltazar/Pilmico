...
public class CenterPanel extends SoyPanel {
	...
	private void fillNULLForm() {
		try {
			...
			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					"NOT APPLICABLE"), BorderLayout.CENTER);
	...
	private void fillExpenses() {
		try {
			...
			String[][] entries = {
					{ "1", "June 20, 2013", "Type A", "John David S. Baltazar" },
					{ "2", "June 18, 2013", "Type B", "Juan dela Cruz" } };
			// String[][] entries = new String[1][headers.length];

			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					Tables.EXPENSES), BorderLayout.CENTER);
	...
	private void fillSales() {
		try {
			...
			String[][] entries = {
					{ "1", "June 20, 2013", "John David S. Baltazar", "More" },
					{ "2", "June 18, 2013", "Juan dela Cruz", "" } };
			// String[][] entries = new String[1][headers.length];

			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					Tables.SALES), BorderLayout.CENTER);
	...
	private void fillAR_Payments() {
		try {
			...
			String[][] entries = {
					{ "1", "June 20, 2012", "John David S. Baltazar", "2500" },
					{ "2", "January 18, 2012", "Juan dela Cruz", "5000" } };
			// String[][] entries = new String[1][headers.length];

			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					Tables.AR_PAYMENTS), BorderLayout.CENTER);
	...
	private void fillAR() {
		try {
			String[] headers = { "ID", "Date", "Issued By", "Receiver",
					"Balance" };
			String[][] entries = {
					{ "1", "June 20, 2013", "John David S. Baltazar",
							"Juan dela Cruz", "500" },
					{ "2", "June 18, 2013", "Juan dela Cruz",
							"John David S. Baltazar", "5000" } };
			// String[][] entries = new String[1][headers.length];

			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					Tables.ACCOUNT_RECEIVABLES), BorderLayout.CENTER);
	...
	private void fillPullout() {
		try {
			String[] headers = { "ID", "Date", "Issued By", "Product",
					"Remarks" };
			String[][] entries = {
					{ "1", "May 20, 2013", "John David S. Baltazar",
							"Hog Feeds", "expired" },
			...
			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					Tables.PULLOUT), BorderLayout.CENTER);
	...
	private void fillDelivery() {
		try {
			...
			String[][] entries = {
					{ "1", "April 18, 2013", "John David S. Baltazar", "" },
					{ "2", "May 11, 2012", "John David S. Baltazar", "Hoax" } };
			// String[][] entries = new String[5][headers.length];

			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					Tables.DELIVERY), BorderLayout.CENTER);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void fillSuppliers() {
		try {
			...
			// String[][] entries = new String[5][headers.length];
			String[][] entries = {
					{ "1", "Rubina Hog Feeds Inc.", "Marikina City", "", "" },
					{ "2", "Universal Feeds Corp.", "Tacloban City",
							"325-5689", "Pugad Baboy" } };

			add(new TableUtilPanel(new TablePanel(entries, headers, null),
					Tables.SUPPLIERS), BorderLayout.CENTER);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void fillProducts() {
		...
		try {
			String[] headers = { "Item No.", "Name", "Quantity (kg)",
					"Quantity (sack)", "Display (kg)", "Display (sack)" };
			// String[][] entries = new String[1][headers.length];

			String[][] entries = { { "1", "Hog Feeds", "25", "5", "15", "5" },
					{ "2", "Chicken Feeds", "30", "10", "5", "3" } };
