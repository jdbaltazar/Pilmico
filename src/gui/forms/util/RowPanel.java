			...
			public void itemStateChanged(ItemEvent arg0) {
				if (productsCombo.getSelectedItem() != null) {
					...
					if (Values.tableUtilPanel.getLabel().contains(Tables.SALES)) {
						...
						Values.salesForm.setProductQuantities(p.getSacks(), p.getKilosOnDisplay());
					}
						...
						Values.accountReceivablesForm.setProductQuantities(p.getSacks(), p.getKilosOnDisplay());
					}
						...
						Values.pulloutForm.setProductQuantities(p.getSacks(), p.getKilosOnDisplay());
					}
