...
public class InventorySheetForm extends SimplePanel {
	...
	private int startX = 15, startY = 17, PANE_WIDTH = 750, PANE_HEIGHT = 380,
			...
			SECTION_GAP = 120, LABEL_GAP = 30, TOTAL_FORMS_OVERALL = 12,
			SCROLLBAR_WIDTH = 16, ROW_HEIGHT = 30;
	/*
	 * private int startX = 15, startY = 17, PANE_WIDTH = 750, PANE_HEIGHT =
	 * 380, TOTAL_LABEL_WIDTH = 100, TABLE_GAP = 50, TAB = 150, SECTION_GAP =
	 * 120, LABEL_GAP = 30, TOTAL_INVENTORY_LABEL = 16, TOTAL_FORMS_OVERALL =
	 * 12, SCROLLBAR_WIDTH = 16;
	 */
	public static int PRODUCT_LABEL_WIDTH = 160, SACK_LABEL_WIDTH = 70,
			SALES_KG_WIDTH = 90, TOTAL_INVENTORY_LABEL = 16,
	...
	private JPanel isPanel, productsPanel, salesPanel, delPanel, arPanel,
			arPaymentPanel, caPaymentPanel, pulloutPanel, expensesPanel,
			CAPanel, discountPanel, ar2Panel, salaryPanel, depositPanel,
			cashBreakdownPanel;
	private JScrollPane isPane, salesPane, delPane, arPane, arPaymentPane,
			caPaymentPane, pulloutPane, expensesPane, CAPane, discountPane,
			ar2Pane, salaryPane, depositPane;
	...
	private TableHeaderLabel productLabel, sack1Label, kg1Label, sack2Label,
			kg2Label, sack3Label, kg3Label, sack4Label, kg4Label, sack5Label,
			kg5Label, sack6Label, kg6Label, sack7Label, kg7Label, sack8Label,
			kg8Label, productTotalLabel, dateSaleslabel, cashierLabel,
			grossSalesLabel, dateDellabel, delReceivedByLabel, grossDelLabel,
			dateARlabel, arIssuedByLabel, grossARLabel, dateARPaymentlabel,
			arPaymentIssuedByLabel, grossARPaymentLabel, dateCAPaymentlabel,
			caPaymentIssuedByLabel, grossCAPaymentLabel, datePulloutlabel,
			pulloutIssuedByLabel, grossPulloutLabel, dateExpenseslabel,
			expensesIssuedByLabel, grossExpensesLabel, dateCAlabel,
			CAIssuedByLabel, grossCALabel, dateDiscountlabel,
			discountAmountLabel, dateAR2label, ar2IssuedByLabel, grossAR2Label,
			dateSalarylabel, salaryIssuedForLabel, salaryAmountLabel,
			dateDepositlabel, depositorLabel, depositAmountLabel;
	private SubTableHeaderLabel begInvtyLabel, onDisplayLabel, delLabel,
			poLabel, endInvtyLabel, offtakeLabel, priceLabel, salesLabel,
			salesFormLabel, overallSalesLabel, arFormLabel, overallARLabel,
			deliveryFormLabel, overallDelLabel, arPaymentFormLabel,
			overallARPaymentLabel, caPaymentFormLabel, overallCAPaymentLabel,
			pullOutFormLabel, overallPulloutLabel, expensesFormLabel,
			overallExpensesLabel, caFormLabel, overallCALabel,
			discountFormLabel, overallDiscountLabel, ar2FormLabel,
			overallAR2Label, salaryFormLabel, overallSalaryLabel,
			depositFormLabel, overallDepositLabel, assetsLabel,
			liabilitiesLabel, pcohLabel, cohLabel, summary1Label,
			summary2Label, summary3Label, accLabel;
	...
	private void init() {
		...
		isPanel.setPreferredSize(new Dimension(PANE_WIDTH + 750,
				PANE_HEIGHT + 2000));
		...
		productsPane.getVerticalScrollBar().addAdjustmentListener(
				new AdjustmentListener() {
					@Override
					public void adjustmentValueChanged(AdjustmentEvent ae) {
						int extent = productsPane.getVerticalScrollBar()
								.getModel().getExtent();
						int value = (productsPane.getVerticalScrollBar()
								.getValue() + extent);
						int max = productsPane.getVerticalScrollBar()
								.getMaximum();
		...
		deliveryFormLabel = new SubTableHeaderLabel("DELIVERIES",
				Color.green.darker());
		...
		arFormLabel = new SubTableHeaderLabel("ACCOUNT RECEIVABLES",
				Color.green.darker());
		...
		arPaymentFormLabel = new SubTableHeaderLabel(
				"ACCOUNT RECEIVABLES PAYMENTS", Color.green.darker());
		...
		caPaymentFormLabel = new SubTableHeaderLabel("CASH ADVANCE PAYMENTS",
				Color.green.darker());
		...
		pullOutFormLabel = new SubTableHeaderLabel("PRODUCT PULLOUTS",
				Color.red.brighter());
		...
		expensesFormLabel = new SubTableHeaderLabel("EXPENSES",
				Color.red.brighter());
		...
		caFormLabel = new SubTableHeaderLabel("CASH ADVANCES",
				Color.red.brighter());
		...
		discountFormLabel = new SubTableHeaderLabel("DISCOUNTS",
				Color.red.brighter());
		...
		ar2FormLabel = new SubTableHeaderLabel("ACCOUNT RECEIVABLES",
				Color.red.brighter());
		...
		salaryFormLabel = new SubTableHeaderLabel("SALARY RELEASE",
				Color.red.brighter());
		...
		depositFormLabel = new SubTableHeaderLabel("DEPOSITS",
				Color.red.brighter());
		...
		liabilitiesLabel = new SubTableHeaderLabel("LIABILITIES",
				Color.red.brighter());
		...
		for (int i = 0; i < 4; i++) {
			...
			computationLabel.get(i).setBorder(
		...
		summary1Label = new SubTableHeaderLabel("CASH ON HAND",
				Color.decode("#A233A2"));
		summary2Label = new SubTableHeaderLabel("ACTUAL CASH COUNT",
				Color.decode("#AE4DAE"));
		...
		for (int i = 0; i < TOTAL_INVENTORY_LABEL; i++) {
			...
			totalInventoryLabel.get(i).setBorder(
		...
		actualCashCount.setBorder(BorderFactory.createMatteBorder(1, 0, 1, 1,
				Color.black));
	...
	private void addComponents() {
		...
		sectionLabel.get(0).setBounds(startX,
				dateLabel.getY() + dateLabel.getHeight() + 30, 150, 20);

		productLabel.setBounds(startX, sectionLabel.get(0).getY()
				+ sectionLabel.get(0).getHeight() + LABEL_GAP,
				PRODUCT_LABEL_WIDTH, 30);

		begInvtyLabel.setBounds(startX + productLabel.getWidth(),
				productLabel.getY(), 140, 10);
		sack1Label.setBounds(startX + productLabel.getWidth() - 1,
				productLabel.getY() + begInvtyLabel.getHeight(),
				begInvtyLabel.getWidth() / 2, 20);
		kg1Label.setBounds(
				startX + productLabel.getWidth() + sack1Label.getWidth() - 2,
				productLabel.getY() + begInvtyLabel.getHeight(),
				begInvtyLabel.getWidth() / 2 + 3, 20);

		onDisplayLabel.setBounds(startX + productLabel.getWidth()
				+ begInvtyLabel.getWidth(), productLabel.getY(), 140, 10);
		sack2Label.setBounds(
				startX + productLabel.getWidth() + begInvtyLabel.getWidth(),
				productLabel.getY() + onDisplayLabel.getHeight(),
				onDisplayLabel.getWidth() / 2, 20);
		kg2Label.setBounds(
				startX + productLabel.getWidth() + begInvtyLabel.getWidth()
						+ sack2Label.getWidth() - 1, productLabel.getY()
						+ onDisplayLabel.getHeight(),
				onDisplayLabel.getWidth() / 2 + 2, 20);

		delLabel.setBounds(
				startX + productLabel.getWidth() + begInvtyLabel.getWidth()
						+ onDisplayLabel.getWidth(), productLabel.getY(), 140,
				10);
		sack3Label.setBounds(onDisplayLabel.getX() + onDisplayLabel.getWidth(),
				productLabel.getY() + delLabel.getHeight(),
				delLabel.getWidth() / 2, 20);
		kg3Label.setBounds(onDisplayLabel.getX() + onDisplayLabel.getWidth()
				+ sack3Label.getWidth() - 1,
				productLabel.getY() + delLabel.getHeight(),
				delLabel.getWidth() / 2 + 2, 20);

		poLabel.setBounds(delLabel.getX() + delLabel.getWidth(),
				productLabel.getY(), 140, 10);
		sack4Label.setBounds(delLabel.getX() + delLabel.getWidth(),
				productLabel.getY() + delLabel.getHeight(),
				delLabel.getWidth() / 2, 20);
		kg4Label.setBounds(
				delLabel.getX() + delLabel.getWidth() + sack3Label.getWidth()
						- 1, productLabel.getY() + delLabel.getHeight(),
				delLabel.getWidth() / 2 + 2, 20);

		endInvtyLabel.setBounds(poLabel.getX() + poLabel.getWidth(),
				productLabel.getY(), 140, 10);
		sack5Label.setBounds(poLabel.getX() + poLabel.getWidth(),
				productLabel.getY() + poLabel.getHeight(),
				poLabel.getWidth() / 2, 20);
		kg5Label.setBounds(
				poLabel.getX() + poLabel.getWidth() + sack5Label.getWidth() - 1,
				productLabel.getY() + poLabel.getHeight(),
		...
		kg6Label.setBounds(endInvtyLabel.getX() + endInvtyLabel.getWidth()
				+ sack6Label.getWidth() - 1, productLabel.getY()
				+ endInvtyLabel.getHeight(), endInvtyLabel.getWidth() / 2 + 2,
				20);
		...
		sack7Label.setBounds(offtakeLabel.getX() + offtakeLabel.getWidth(),
				productLabel.getY() + offtakeLabel.getHeight(),
				priceLabel.getWidth() / 2, 20);
		kg7Label.setBounds(offtakeLabel.getX() + offtakeLabel.getWidth()
				+ sack7Label.getWidth() - 1,
				productLabel.getY() + offtakeLabel.getHeight(),
		...
		sack8Label.setBounds(priceLabel.getX() + priceLabel.getWidth(),
				productLabel.getY() + priceLabel.getHeight(),
				salesLabel.getWidth() / 2, 20);
		kg8Label.setBounds(priceLabel.getX() + priceLabel.getWidth()
				+ sack8Label.getWidth() - 1,
				productLabel.getY() + priceLabel.getHeight(),
		...
		productTotalLabel.setBounds(startX,
				productsPane.getY() + productsPane.getHeight(),
				productLabel.getWidth(), 20);

		fillTables();
		for (int i = 0, x = 0; i < TOTAL_INVENTORY_LABEL - 4; i++, x += sack1Label
				.getWidth())
			totalInventoryLabel.get(i).setBounds(
					productTotalLabel.getX() + productTotalLabel.getWidth() + x
							- 1, productTotalLabel.getY(),
					sack1Label.getWidth(), 20);

		for (int i = TOTAL_INVENTORY_LABEL - 4, x = 0; i < TOTAL_INVENTORY_LABEL; i++, x += kg7Label
				.getWidth())
			totalInventoryLabel.get(i).setBounds(
					kg6Label.getX() + kg6Label.getWidth() + x - 2,
					productTotalLabel.getY(), kg7Label.getWidth(), 20);

		sectionLabel.get(1).setText("ASSETS");
		sectionLabel.get(1).setBounds(startX,
				productsPane.getY() + productsPane.getHeight() + SECTION_GAP,
				150, 20);

		salesFormLabel.setBounds(startX, sectionLabel.get(1).getY()
				+ sectionLabel.get(1).getHeight() + LABEL_GAP, 450, 18);
		dateSaleslabel.setBounds(startX - 1, salesFormLabel.getY()
				+ salesFormLabel.getHeight(), 150, 20);
		cashierLabel.setBounds(startX + dateSaleslabel.getWidth() - 2,
				salesFormLabel.getY() + salesFormLabel.getHeight(),
				salesFormLabel.getWidth() - dateSaleslabel.getWidth() + 2
						- TOTAL_LABEL_WIDTH, 20);
		grossSalesLabel.setBounds(cashierLabel.getX() + cashierLabel.getWidth()
				- 1, salesFormLabel.getY() + salesFormLabel.getHeight(),
				salesFormLabel.getWidth()
						- (dateSaleslabel.getWidth() + cashierLabel.getWidth())
						+ 4, 20);
		salesPane.setBounds(startX,
				dateSaleslabel.getY() + dateSaleslabel.getHeight() - 1,
				salesFormLabel.getWidth() + SCROLLBAR_WIDTH, 105);
		overallSalesLabel.setBounds(startX,
				salesPane.getY() + salesPane.getHeight(), 200, 15);
		formsOverall.get(0).setText("P 15890.50");
		formsOverall.get(0).setBounds(
				overallSalesLabel.getX() + overallSalesLabel.getWidth(),
				overallSalesLabel.getY(),
				salesPane.getWidth() - overallSalesLabel.getWidth(), 15);

		deliveryFormLabel.setBounds(startX + salesFormLabel.getWidth()
				+ TABLE_GAP, salesFormLabel.getY(), 450, 18);
		dateDellabel.setBounds(deliveryFormLabel.getX() - 1,
				deliveryFormLabel.getY() + deliveryFormLabel.getHeight(), 150,
				20);
		delReceivedByLabel.setBounds(
				deliveryFormLabel.getX() + dateDellabel.getWidth() - 2,
				deliveryFormLabel.getY() + deliveryFormLabel.getHeight(),
				deliveryFormLabel.getWidth() - dateDellabel.getWidth() + 2
						- TOTAL_LABEL_WIDTH, 20);
		grossDelLabel.setBounds(
				delReceivedByLabel.getX() + delReceivedByLabel.getWidth() - 1,
				deliveryFormLabel.getY() + deliveryFormLabel.getHeight(),
				deliveryFormLabel.getWidth()
						- (dateDellabel.getWidth() + delReceivedByLabel
								.getWidth()) + 4, 20);
		delPane.setBounds(startX + salesFormLabel.getWidth() + TABLE_GAP,
				dateDellabel.getY() + dateDellabel.getHeight() - 1,
				salesFormLabel.getWidth(), 105);
		overallDelLabel.setBounds(startX + salesFormLabel.getWidth()
				+ TABLE_GAP, delPane.getY() + delPane.getHeight(), 200, 15);
		formsOverall.get(1).setBounds(
				overallDelLabel.getX() + overallDelLabel.getWidth(),
				overallDelLabel.getY(),
				delPane.getWidth() - overallDelLabel.getWidth(), 15);

		arFormLabel.setBounds(
				deliveryFormLabel.getX() + deliveryFormLabel.getWidth()
						+ TABLE_GAP, salesFormLabel.getY(), 450, 18);
		dateARlabel.setBounds(arFormLabel.getX() - 1, arFormLabel.getY()
				+ arFormLabel.getHeight(), 150, 20);
		arIssuedByLabel.setBounds(arFormLabel.getX() + dateARlabel.getWidth()
				- 2, arFormLabel.getY() + arFormLabel.getHeight(),
				arFormLabel.getWidth() - dateARlabel.getWidth() + 2
						- TOTAL_LABEL_WIDTH, 20);
		grossARLabel.setBounds(
				arIssuedByLabel.getX() + arIssuedByLabel.getWidth() - 1,
				arFormLabel.getY() + arFormLabel.getHeight(),
				arFormLabel.getWidth()
						- (dateARlabel.getWidth() + arIssuedByLabel.getWidth())
						+ 4, 20);
		arPane.setBounds(
				deliveryFormLabel.getX() + deliveryFormLabel.getWidth()
						+ TABLE_GAP,
				dateARlabel.getY() + dateARlabel.getHeight() - 1,
				arFormLabel.getWidth(), 105);
		overallARLabel.setBounds(
				deliveryFormLabel.getX() + deliveryFormLabel.getWidth()
						+ TABLE_GAP, delPane.getY() + delPane.getHeight(), 200,
				15);
		formsOverall.get(2).setBounds(
				overallARLabel.getX() + overallARLabel.getWidth(),
				overallARLabel.getY(),
		...
		arPaymentIssuedByLabel.setBounds(arPaymentFormLabel.getX()
				+ dateARPaymentlabel.getWidth() - 2, arPaymentFormLabel.getY()
				+ arPaymentFormLabel.getHeight(), arPaymentFormLabel.getWidth()
				- dateARPaymentlabel.getWidth() + 2 - TOTAL_LABEL_WIDTH, 20);
		grossARPaymentLabel
				.setBounds(
						arPaymentIssuedByLabel.getX()
								+ arPaymentIssuedByLabel.getWidth() - 1,
						arPaymentFormLabel.getY()
								+ arPaymentFormLabel.getHeight(),
						arPaymentFormLabel.getWidth()
								- (dateARPaymentlabel.getWidth() + arPaymentIssuedByLabel
										.getWidth()) + 4, 20);
		arPaymentPane.setBounds(startX + TAB, dateARPaymentlabel.getY()
				+ dateARPaymentlabel.getHeight() - 1,
				arPaymentFormLabel.getWidth(), 105);
		overallARPaymentLabel.setBounds(startX + TAB, arPaymentPane.getY()
				+ arPaymentPane.getHeight(), 200, 15);
		formsOverall.get(3)
				.setBounds(
						overallARPaymentLabel.getX()
								+ overallARPaymentLabel.getWidth(),
						overallARPaymentLabel.getY(),
						arPaymentPane.getWidth()
								- overallARPaymentLabel.getWidth(), 15);

		caPaymentFormLabel.setBounds(arPaymentFormLabel.getX()
				+ arPaymentFormLabel.getWidth() + TABLE_GAP + TAB,
				arPaymentFormLabel.getY(), 450, 18);
		...
		caPaymentIssuedByLabel.setBounds(caPaymentFormLabel.getX()
				+ dateCAPaymentlabel.getWidth() - 2, caPaymentFormLabel.getY()
				+ caPaymentFormLabel.getHeight(), caPaymentFormLabel.getWidth()
				- dateCAPaymentlabel.getWidth() + 2 - TOTAL_LABEL_WIDTH, 20);
		grossCAPaymentLabel
				.setBounds(
						caPaymentIssuedByLabel.getX()
								+ caPaymentIssuedByLabel.getWidth() - 1,
						caPaymentFormLabel.getY()
								+ arPaymentFormLabel.getHeight(),
						caPaymentFormLabel.getWidth()
								- (dateCAPaymentlabel.getWidth() + caPaymentIssuedByLabel
										.getWidth()) + 4, 20);
		caPaymentPane.setBounds(caPaymentFormLabel.getX(),
				dateCAPaymentlabel.getY() + dateCAPaymentlabel.getHeight() - 1,
		...
		formsOverall.get(4)
				.setBounds(
						overallCAPaymentLabel.getX()
								+ overallCAPaymentLabel.getWidth(),
						overallCAPaymentLabel.getY(),
						caPaymentPane.getWidth()
								- overallCAPaymentLabel.getWidth(), 15);

		sectionLabel.get(2).setText("LIABILITIES");
		sectionLabel.get(2).setBounds(
				startX,
				overallCAPaymentLabel.getY()
						+ overallCAPaymentLabel.getHeight() + SECTION_GAP, 150,
				20);

		pullOutFormLabel.setBounds(startX + TAB, sectionLabel.get(2).getY()
				+ sectionLabel.get(2).getHeight() + LABEL_GAP, 450, 18);
		datePulloutlabel
				.setBounds(pullOutFormLabel.getX() - 1, pullOutFormLabel.getY()
						+ pullOutFormLabel.getHeight(), 150, 20);
		pulloutIssuedByLabel.setBounds(pullOutFormLabel.getX()
				+ datePulloutlabel.getWidth() - 2, pullOutFormLabel.getY()
				+ pullOutFormLabel.getHeight(), pullOutFormLabel.getWidth()
				- datePulloutlabel.getWidth() + 2 - TOTAL_LABEL_WIDTH, 20);
		grossPulloutLabel.setBounds(
				pulloutIssuedByLabel.getX() + pulloutIssuedByLabel.getWidth()
						- 1,
				pullOutFormLabel.getY() + pullOutFormLabel.getHeight(),
				pullOutFormLabel.getWidth()
						- (datePulloutlabel.getWidth() + pulloutIssuedByLabel
								.getWidth()) + 4, 20);
		pulloutPane.setBounds(pullOutFormLabel.getX(), datePulloutlabel.getY()
				+ datePulloutlabel.getHeight() - 1,
				pullOutFormLabel.getWidth(), 105);
		overallPulloutLabel.setBounds(pullOutFormLabel.getX(),
				pulloutPane.getY() + pulloutPane.getHeight(), 200, 15);
		formsOverall.get(5).setBounds(
				overallPulloutLabel.getX() + overallPulloutLabel.getWidth(),
				overallPulloutLabel.getY(),
				pulloutPane.getWidth() - overallPulloutLabel.getWidth(), 15);

		expensesFormLabel.setBounds(
				pullOutFormLabel.getX() + pullOutFormLabel.getWidth()
						+ TABLE_GAP + TAB, pullOutFormLabel.getY(), 450, 18);
		...
		expensesIssuedByLabel.setBounds(expensesFormLabel.getX()
				+ dateExpenseslabel.getWidth() - 2, expensesFormLabel.getY()
				+ expensesFormLabel.getHeight(), expensesFormLabel.getWidth()
				- dateExpenseslabel.getWidth() + 2 - TOTAL_LABEL_WIDTH, 20);
		grossExpensesLabel.setBounds(
				expensesIssuedByLabel.getX() + expensesIssuedByLabel.getWidth()
						- 1,
				expensesFormLabel.getY() + expensesFormLabel.getHeight(),
				expensesFormLabel.getWidth()
						- (dateExpenseslabel.getWidth() + expensesIssuedByLabel
								.getWidth()) + 4, 20);
		expensesPane.setBounds(expensesFormLabel.getX(),
				dateExpenseslabel.getY() + dateExpenseslabel.getHeight() - 1,
				expensesFormLabel.getWidth(), 105);
		overallExpensesLabel.setBounds(expensesFormLabel.getX(),
				expensesPane.getY() + expensesPane.getHeight(), 200, 15);
		formsOverall.get(6).setBounds(
				overallExpensesLabel.getX() + overallExpensesLabel.getWidth(),
				overallExpensesLabel.getY(),
		...
		CAIssuedByLabel.setBounds(caFormLabel.getX() + dateCAlabel.getWidth()
				- 2, caFormLabel.getY() + caFormLabel.getHeight(),
				caFormLabel.getWidth() - dateCAlabel.getWidth() + 2
						- TOTAL_LABEL_WIDTH, 20);
		grossCALabel.setBounds(
				CAIssuedByLabel.getX() + CAIssuedByLabel.getWidth() - 1,
				caFormLabel.getY() + caFormLabel.getHeight(),
				caFormLabel.getWidth()
						- (dateCAlabel.getWidth() + CAIssuedByLabel.getWidth())
						+ 4, 20);
		CAPane.setBounds(caFormLabel.getX(),
				dateCAlabel.getY() + dateCAlabel.getHeight() - 1,
				caFormLabel.getWidth(), 105);
		overallCALabel.setBounds(caFormLabel.getX(),
				CAPane.getY() + CAPane.getHeight(), 200, 15);
		formsOverall.get(7).setBounds(
				overallCALabel.getX() + overallCALabel.getWidth(),
				overallCALabel.getY(),
				CAPane.getWidth() - overallCALabel.getWidth(), 15);

		discountFormLabel.setBounds(caFormLabel.getX() + caFormLabel.getWidth()
				+ 100,
				overallExpensesLabel.getY() + overallExpensesLabel.getHeight()
						+ 70, 250, 18);
		dateDiscountlabel.setBounds(discountFormLabel.getX() - 1,
				discountFormLabel.getY() + discountFormLabel.getHeight(), 150,
				20);
		discountAmountLabel.setBounds(discountFormLabel.getX()
				+ dateDiscountlabel.getWidth() - 2, discountFormLabel.getY()
				+ discountFormLabel.getHeight(), discountFormLabel.getWidth()
				- dateDiscountlabel.getWidth() + 3, 20);
		discountPane.setBounds(discountFormLabel.getX(),
				dateDiscountlabel.getY() + dateDiscountlabel.getHeight() - 1,
				discountFormLabel.getWidth(), 105);
		overallDiscountLabel.setBounds(discountFormLabel.getX(),
				discountPane.getY() + discountPane.getHeight(), 100, 15);
		formsOverall.get(8).setBounds(
				overallDiscountLabel.getX() + overallDiscountLabel.getWidth(),
				overallDiscountLabel.getY(),
				discountPane.getWidth() - overallDiscountLabel.getWidth(), 15);

		ar2FormLabel.setBounds(
				discountFormLabel.getX() + discountFormLabel.getWidth() + 100,
				caFormLabel.getY(), 450, 18);
		dateAR2label.setBounds(ar2FormLabel.getX() - 1, ar2FormLabel.getY()
				+ ar2FormLabel.getHeight(), 150, 20);
		ar2IssuedByLabel.setBounds(
				ar2FormLabel.getX() + dateAR2label.getWidth() - 2,
				ar2FormLabel.getY() + ar2FormLabel.getHeight(),
				ar2FormLabel.getWidth() - dateAR2label.getWidth() + 2
						- TOTAL_LABEL_WIDTH, 20);
		grossAR2Label.setBounds(
				ar2IssuedByLabel.getX() + ar2IssuedByLabel.getWidth() - 1,
				ar2FormLabel.getY() + ar2FormLabel.getHeight(),
				ar2FormLabel.getWidth()
						- (dateAR2label.getWidth() + ar2IssuedByLabel
								.getWidth()) + 4, 20);
		ar2Pane.setBounds(ar2FormLabel.getX(), dateAR2label.getY()
				+ dateAR2label.getHeight() - 1, ar2FormLabel.getWidth(), 105);
		overallAR2Label.setBounds(ar2FormLabel.getX(),
				ar2Pane.getY() + ar2Pane.getHeight(), 200, 15);
		formsOverall.get(9).setBounds(
				overallAR2Label.getX() + overallAR2Label.getWidth(),
				overallAR2Label.getY(),
		...
		dateSalarylabel.setBounds(salaryFormLabel.getX() - 1,
				salaryFormLabel.getY() + salaryFormLabel.getHeight(), 150, 20);
		salaryIssuedForLabel.setBounds(salaryFormLabel.getX()
				+ datePulloutlabel.getWidth() - 2, salaryFormLabel.getY()
				+ salaryFormLabel.getHeight(), salaryFormLabel.getWidth()
				- dateSalarylabel.getWidth() + 2 - TOTAL_LABEL_WIDTH, 20);
		salaryAmountLabel.setBounds(
				salaryIssuedForLabel.getX() + salaryIssuedForLabel.getWidth()
						- 1,
				salaryFormLabel.getY() + salaryFormLabel.getHeight(),
				salaryFormLabel.getWidth()
						- (dateSalarylabel.getWidth() + salaryIssuedForLabel
								.getWidth()) + 4, 20);
		...
		formsOverall.get(10).setBounds(
				overallSalaryLabel.getX() + overallSalaryLabel.getWidth(),
				overallSalaryLabel.getY(),
				salaryPane.getWidth() - overallSalaryLabel.getWidth(), 15);

		depositFormLabel.setBounds(
				salaryFormLabel.getX() + salaryFormLabel.getWidth() + TABLE_GAP
						+ TAB, salaryFormLabel.getY(), 450, 18);
		dateDepositlabel
				.setBounds(depositFormLabel.getX() - 1, depositFormLabel.getY()
						+ depositFormLabel.getHeight(), 150, 20);
		depositorLabel.setBounds(
				depositFormLabel.getX() + dateDepositlabel.getWidth() - 2,
				depositFormLabel.getY() + depositFormLabel.getHeight(),
				depositFormLabel.getWidth() - dateDepositlabel.getWidth() + 2
						- TOTAL_LABEL_WIDTH, 20);
		depositAmountLabel.setBounds(
				depositorLabel.getX() + depositorLabel.getWidth() - 1,
				depositFormLabel.getY() + depositFormLabel.getHeight(),
				depositFormLabel.getWidth()
						- (dateExpenseslabel.getWidth() + depositorLabel
								.getWidth()) + 4, 20);
		depositPane.setBounds(depositFormLabel.getX(), dateDepositlabel.getY()
				+ dateDepositlabel.getHeight() - 1,
				depositFormLabel.getWidth(), 105);
		overallDepositLabel.setBounds(depositFormLabel.getX(),
				depositPane.getY() + depositPane.getHeight(), 200, 15);
		formsOverall.get(11).setBounds(
				overallDepositLabel.getX() + overallDepositLabel.getWidth(),
				overallDepositLabel.getY(),
				depositPane.getWidth() - overallDepositLabel.getWidth(), 15);

		sectionLabel.get(3).setText("CASH ON HAND");
		sectionLabel.get(3).setBounds(
				startX,
				overallDepositLabel.getY() + overallDepositLabel.getHeight()
						+ SECTION_GAP, 150, 20);

		pcohLabel.setBounds(startX + TAB, sectionLabel.get(3).getY()
				+ sectionLabel.get(3).getHeight() + LABEL_GAP, 150, 25);
		assetsLabel.setBounds(pcohLabel.getX(),
				pcohLabel.getY() + pcohLabel.getHeight(), 150, 25);
		...
		computationLabel.get(0).setBounds(
				pcohLabel.getX() + pcohLabel.getWidth() - 1, pcohLabel.getY(),
				200, 26);
		...
		cashBreakdownPanel.setBounds(computationLabel.get(0).getX()
				+ computationLabel.get(0).getWidth() + TABLE_GAP + TAB + TAB
				+ 50, sectionLabel.get(3).getY()
				+ sectionLabel.get(3).getHeight() + LABEL_GAP, 340, 210);

		sectionLabel.get(4).setText("CASH BREAKDOWN");
		sectionLabel.get(4).setBounds(
				cashBreakdownPanel.getX() - TAB,
				overallDepositLabel.getY() + overallDepositLabel.getHeight()
						+ SECTION_GAP, 150, 20);
		...
		accLabel.setBounds(cashBreakdownPanel.getX(),
				cashBreakdownPanel.getHeight() + cashBreakdownPanel.getY(),
				200, 30);
		actualCashCount.setBounds(accLabel.getX() + accLabel.getWidth(),
				accLabel.getY(),
				cashBreakdownPanel.getWidth() - accLabel.getWidth(), 30);

		sectionLabel.get(5).setText("SUMMARY");
		sectionLabel.get(5).setBounds(startX,
				cohLabel.getY() + cohLabel.getHeight() + SECTION_GAP, 150, 20);

		summary1Label.setBounds(startX + TAB, sectionLabel.get(5).getY()
				+ sectionLabel.get(5).getHeight() + LABEL_GAP, 200, 25);
		summary2Label.setBounds(
				summary1Label.getX() + summary1Label.getWidth(),
				summary1Label.getY(), 200, 25);
		summary3Label.setBounds(
				summary2Label.getX() + summary2Label.getWidth(),
				summary1Label.getY(), 201, 25);

		for (int i = 0, x = 0; i < 3; i++, x += summary1Label.getWidth())
			summaryValues.get(i).setBounds(summary1Label.getX() + x,
					summary1Label.getY() + summary1Label.getHeight(), 201, 30);
		...
		navigationPanel.setBounds(isPane.getWidth() - 190, isPane.getY()
				+ isPane.getHeight() - 100, 200, 100);
	...
	private void fillTables() {
		...
		for (int i = 0; i < 28; i++) {
			...
			productsPanel
					.setPreferredSize(new Dimension(productsPanel.getWidth(),
							productsPanel.getComponentCount() * ROW_HEIGHT));
		...
		for (int i = 0; i < 3; i++) {
			sales.add(new ISRowPanel(null, salesPanel,
					Values.SALES));
			salesPanel.add(sales.get(i));

			salesPanel
					.setPreferredSize(new Dimension(salesPanel.getWidth(),
	...
	private void alternateRows(ArrayList<ISRowPanel> rowPanel) {
		...
		for (int i = 0; i < rowPanel.size(); i++)
			if (i % 2 == 0)
				rowPanel.get(i).getRow()
						.setBackground(Values.row1);
			else
				rowPanel.get(i).getRow()
						.setBackground(Values.row2);
	}
